...
class WaterBalanceClass():
    ...
    def initializeModuleforRun(self, startTimeIndex):
        ...
        self.endResult_df = pd.DataFrame({'Fieldwl':[self._outPut_df['Fieldwl_Final'][0]]}, index= [self._rf_evap_df.index[loc_step]])
        self.endResult_df['Rootzone_wl'] =self._outPut_df['Rootzone_wl_Final'][0]
        self.endResult_df['Subsoilwl'] =self._outPut_df['Subsoilwl_Final'][0]
        
        #Network step 8

        self._outPut_df['Discharge_network_np'] = (self._outPut_df['Fieldwl_s5']-self._rf_evap_df['WL'][loc_step]) # water level is variable here
        self._outPut_df['Efficiency_network_np']=pd.DataFrame({'s0':(-1)* self._outPut_df['Days']*float(self._waterBalanceParameter_df['MaxDrainagerate'][1]), 's1': pd.DataFrame( {'s1': self._outPut_df['Discharge_network']* float(self._waterBalanceParameter_df['Drainage_eff'][1])/100 , 's2':self._outPut_df['Days']*float(self._waterBalanceParameter_df['MaxDrainagerate'][1])}).min(axis=1)}).max(axis=1) * self._outPut_df['Gate_O_C'] # replace by upazilla code
        self._outPut_df['Fieldwl_network_np'] = self._outPut_df['Fieldwl_s5']-self._outPut_df['Efficiency_network']
        
        self.endResult_df['Fieldwl_np'] =self._outPut_df['Fieldwl_network_np'][0]
        self.endResult_df['Rootzone_wl_np'] =self._outPut_df['Rootzonewl_s7'][0]
        self.endResult_df['Subsoilwl_np'] =self._outPut_df['Subsoilwl_s7'][0]
    ...
    def dotimeStep(self, startTimeIndex, endTimeIndex):
        ...
        for i in range(1, maxNoTimeStep+1): #self._rf_evap_df.shape[0]-1000
            ...
            #Network for non project step Step extra
            row.append(self.gateOperation(self._rf_evap_df.index[loc_step])) # 45 Gate_O_C'
            row.append((row[18]-self._rf_evap_df['WL'][loc_step])*row[24]) # 46 Discharge_network ---- water level is variable here
            row.append(max(-1*row[1]* float(self._waterBalanceParameter_df['MaxDrainagerate'][1]), min(row[25]* int(self._waterBalanceParameter_df['Drainage_eff'][1])/100 , row[1]*float(self._waterBalanceParameter_df['MaxDrainagerate'][1])))* row[24])  # 47 Efficiency_network---- replace by upazilla code
            row.append( row[18]-row[26]) # 48 Fieldwl_network
            self._outPut_df.loc[date] = row
        self._outPut_df.to_csv('output.csv', sep=',')
        #print(self._outPut_df.index.get_loc('1985-06-11 00:00:00')) #[['11/01/1985']])
